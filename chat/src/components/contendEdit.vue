<template>
  <div class="editContentCtn">
    <div id="messageDesc" contenteditable="true" v-html="parseContendMsg" @keyup="contenteditableKeyUp"
      @keydown="contenteditableKeyDown" @blur="contendEditBlur" @click="contendEditClick">
    </div>
    <div v-if="showContentEdit" class="selectCtnBgCover" @click="selectCtnBgCoverClick"></div>
    <ContendEditSelectList v-if="showContentEdit" :left="left" :selectDataList="selectDataList"
      :selectItemIndex="selectItemIndex" @itemClick="itemClick" />
  </div>
</template>
    ...
    documentKeyMethod(e) {
      ...
      if (this.showContentEdit) {
        if (e.keyCode === 38 || e.keyCode === 40 || e.keyCode === 37 || e.keyCode === 39) {
          ...
          if (e.keyCode === 38 || e.keyCode === 37) { // 上键
            this.selectItemIndex = (index - 1) <= 0 ? 1 : index - 1
          } else if (e.keyCode === 40 || e.keyCode === 39) { // 到底之后 重新开始
