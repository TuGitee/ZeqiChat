...
const ExgType = {
    img: new RegExp(`${leftArrow}img src="((blob:)?http(s)?:\/\/.+|\/uploads\/.*?\.webp)" ?\/?${rightArrow}`, 'g'),
    a: new RegExp(`${leftArrow}a href="(http(s)?:\/\/.+)"${rightArrow}(.*)${leftArrow}\/?a${rightArrow}`, 'g'),
    video: new RegExp(`${leftArrow}video src="((blob:)?http(s)?:\/\/.+|\/uploads\/.*?)"${rightArrow}${leftArrow}\/?video${rightArrow}`, 'g'),
    audio: new RegExp(`${leftArrow}audio src="((blob:)?http(s)?:\/\/.+|\/uploads\/.*?)"${rightArrow}${leftArrow}\/?audio${rightArrow}`, 'g'),
    ...
    firework: new RegExp(`${leftArrow}firework${rightArrow}(.*?)${leftArrow}\/?firework${rightArrow}`, 'g'),
}

function formatMessage(msg) {
    for (const reg in ExgType) {
        if (ExgType[reg].test(msg)) {
            msg = msg.trim().replace(ExgType[reg], (match, p1, p2, p3, p4) => {
                switch (reg) {
                    case 'img':
                        if (p1) {
                            if (/^\/uploads/.test(p1)) return `<img src="http://47.120.2.219:3000${p1}" alt="404 Not Found" />`
                            return `<img src="${p1}" />`
                        }
                        else return `${leftArrow}${reg} alt="404 Not Found"${rightArrow}${leftArrow}/${reg}${rightArrow}`
                    case 'a':
                        if (!p1) return `${match}`
                    ...
                    case 'video': case 'audio':
                        if (!p1) return `${match}`
                        if (/^\/uploads/.test(p1)) return `<${reg} src="http://47.120.2.219:3000${p1}" type="audio/mpeg" controls ></${reg}>`;
                        return `<${reg} controls src="${p1}" type="audio/mpeg"></${reg}>`;
                    case 'hr': case 'md_hr':
                        return `<hr />`;
    ...
    msg = msg?.replace(new RegExp(newLine, "g"), '<br/>')?.replace(new RegExp(leftArrow, "g"), '&lt;')?.replace(new RegExp(rightArrow, "g"), '&gt;');
...
export { formatMessage, removeSlash }