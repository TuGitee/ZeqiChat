...
import iconfont from '@/less/emoji/iconfont.js'
...
const ExgType = {
    ...
    audio: new RegExp(`&lt;audio src="((blob:)?(https?|file):\/\/.+|\/uploads\/.*?)"(.*?)&gt;&lt;\/?audio&gt;`, 'g'),
...
function formatMessage(msg) {
        ...
        if (ExgType[reg].test(msg)) {
            msg = msg.trim().replace(ExgType[reg], (match, p1, p2, p3, p4) => {
                switch (reg) {
                    ...
                    case 'emoji':
                        const emoji = iconfont.find(icon => icon.name === p1 || icon.id === p1 || icon.font_class === p1)
                        if (!emoji) return `${match}`
                        return `<i class="iconfont icon-${emoji.font_class}"></i>`
    ...
    if (msg?.trim()) {
        const md = marked(msg)
        return /^<p>[\s\S]*?<\/p>[ \n]?$/.test(md.trim()) ? md.replace(/\n/g, '<br/>').replace(/(.*?)<br\/>$/, '$1').replace(/(<\/.*?>)<br\/>(<.*?>)/g, '$1$2') : md
    }
