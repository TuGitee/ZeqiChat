...
const ExgType = {
    ...
    audio: new RegExp(`&lt;audio src="((blob:)?http(s)?:\/\/.+|\/uploads\/.*?)"(.*?)&gt;&lt;\/?audio&gt;`, 'g'),

    color: new RegExp(`&lt;color(:|=)"?(.*?)"?&gt;(.*?)&lt;\/?color&gt;`, 'g'),
    size: new RegExp(`&lt;size(:|=)"?(.*?)"?&gt;(.*?)&lt;\/?size&gt;`, 'g'),

    firework: new RegExp(`&lt;firework&gt;(.*?)&lt;\/?firework&gt;`, 'g'),
}

function formatMessage(msg) {
    msg = msg?.replace(/<(.*?)>(.*?)<\/?(.*?)>/g, '&lt;$1&gt;$2&lt;\/$3&gt;').replace(/<(.*?)\/>/g, '&lt;$1\/&gt;').replace(/<(iframe|a)/g,'&lt;$1').replace(/```(.*?)\n(.*?)\n```/g,(match,p1,p2)=>{
        ...
        if (ExgType[reg].test(msg)) {
            msg = msg.trim().replace(ExgType[reg], (match, p1, p2, p3,p4) => {
                switch (reg) {
                    ...
                    case 'audio':
                        if (!p1) return `${match}`
                        if (/^\/uploads/.test(p1)) return `<${reg} src="https://zeqichat.xyz${p1}" type="audio/mpeg" controls ${p4} ></${reg}>`;
                        return `<${reg} controls src="${p1}" type="audio/mpeg" ${p4}></${reg}>`;
    ...
    if (msg?.trim()) {
        const md = marked(msg)
        const match = md.replace(/<p>|<\/p>/g, '')
        if (match) return match === msg ? msg.replace(/\n/g, '<br/>') : match
    }
...
export { formatMessage, filterMessage }